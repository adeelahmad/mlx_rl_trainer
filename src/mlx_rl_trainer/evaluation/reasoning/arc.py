...
from mlx_rl_trainer.utils.mlx_utils import _create_4d_attention_mask, safe_make_sampler # For generation utils
...
class ARCEvaluator(BaseEvaluator):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.gen_temp = config.get("temperature", 0.0) # Use configured temp, fallback to greedy
    ...
    def evaluate(self, model: Any, tokenizer: TokenizerWrapper, dataset: Dataset) -> EvaluationMetrics:
        ...
        for problem_idx, problem in enumerate(sampled_problems):
            ...
            if not prompt or not choices['text'] or not correct_label: continue # Skip if missing crucial info
            ...
            if predicted_answer_label is not None and predicted_answer_label == correct_label:
                correct_count += 1

        accuracy = float(correct_count / total_problems)
        logger.info(f"ARC Evaluation: Accuracy = {accuracy:.4f} ({correct_count}/{total_problems})")

        return EvaluationMetrics(task_name=self.name, pass_rate=accuracy, additional_info={"num_samples": total_problems})

    def _generate_solution(self, model: Any, tokenizer: TokenizerWrapper, prompt: str, choices: Dict[str, List[str]]) -> str:
        ...
        # Apply chat template
        formatted_full_prompt = apply_chat_template_wrapper(tokenizer, full_prompt, self.system_prompt)

        encoded_prompt = tokenizer.encode(formatted_full_prompt, add_special_tokens=True)
        prompt_mx = mx.array([encoded_prompt], dtype=mx.int32)

        # Initialize KV cache
        caches = cache.make_prompt_cache(model, max_kv_size=prompt_mx.shape[1] + self.max_gen_len)
        ...
        # Sampler
        sampler = safe_make_sampler(self.config, temp=self.gen_temp) # Pass eval config as args
        ...
        for _ in range(self.max_gen_len):
            ...
            for proc_fn in logits_processors:
                logits_to_process = proc_fn([current_ids], logits_to_process) # Pass list of lists for batching
    ...
    def _extract_arc_answer(self, generated_text: str, choices: Dict[str, List[str]]) -> Optional[str]:
