...
class ARCEvaluator(BaseEvaluator):
    ...
    def evaluate(self, model: Any, tokenizer: TokenizerWrapper, dataset: Dataset) -> EvaluationMetrics:
        ...
        for problem in sampled_problems:
            prompt, choices = problem.get("prompt", ""), problem.get("choices", {"text": [], "label": []})
            correct_label = problem.get("answerKey", "")
            if not prompt or not choices["text"] or not correct_label: continue
            ...
            if predicted_label == correct_label: correct_count += 1
    ...
    def _generate_solution(self, model: Any, tokenizer: TokenizerWrapper, prompt: str, choices: Dict[str, List[str]]) -> str:
        ...
        model.eval()
        formatted_prompt = apply_chat_template_wrapper(tokenizer, full_prompt, self.system_prompt)
