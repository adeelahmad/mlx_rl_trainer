...
from mlx_rl_trainer.core.exceptions import CheckpointError # CORRECTED IMPORT
...
def _is_metal_internal_error(err: BaseException) -> bool:
    s = str(err)
    return "Command buffer execution failed" in s or "[METAL]" in s or "Internal Error" in s
...
def _find_embedding_layer(root: nn.Module) -> Tuple[str, nn.Module]:
    for name, mod in root.named_modules():
        if isinstance(mod, (nn.Embedding, nn.QuantizedEmbedding)): return name, mod
    raise RuntimeError('No nn.Embedding layer found.')

def _freeze_module(module: nn.Module):
    if module:
        for p in module.parameters(): p.flags.train = False

class ContentAlignBridge(nn.Module):
    def __init__(self, teacher_model:nn.Module, student_model:nn.Module, teacher_tokenizer:TokenizerWrapper, student_tokenizer:TokenizerWrapper, bridge_path:str, pool:str='mean', scale:float=1., gen_cfg:Optional[GenerationConfig]=None):
        super().__init__()
        from mlx_rl_trainer.utils.text_utils import extract_answer_region # Local import to avoid cycles
        self.tok_t = teacher_tokenizer
        self.tok_s = student_tokenizer
        self.pool = pool
        ...
        t_dim = int(self.t_emb.weight.shape[1])
        s_dim = int(self.s_emb.weight.shape[1])
        hidden = max(t_dim, s_dim)
        self.bridge = nn.Sequential(nn.Linear(t_dim, hidden, bias=False), nn.ReLU(), nn.Linear(hidden, s_dim, bias=False))
        try:
    ...
    def _pool_vec(tok_emb: mx.array, pool: str) -> mx.array:
        if tok_emb.size == 0: return mx.zeros((tok_emb.shape[-1],), dtype=tok_emb.dtype)
        if pool == 'last': return tok_emb[-1]
        return tok_emb.mean(axis=0)
    ...
    def __call__(self, texts: List[str]) -> List[float]:
        ...
        for s in texts:
            ...
            t_ids = self.tok_t.encode(ans, add_special_tokens=False) or []
            s_ids = self.tok_s.encode(ans, add_special_tokens=False) or []
            if not t_ids or not s_ids: scores.append(0.0); continue
            t_vec = self._pool_vec(self.t_emb(mx.array(t_ids, dtype=mx.int32)), self.pool)
            s_vec = self._pool_vec(self.s_emb(mx.array(s_ids, dtype=mx.int32)), self.pool)
            mapped = self.bridge(t_vec)
            a = mapped / (mx.norm(mapped) + 1e-08)
            b = s_vec / (mx.norm(s_vec) + 1e-08)
            cos = mx.sum(a * b)
            score = 0.5 * (1. + cos)
            score = float(mx.clip(score, 0.0, 1.0).item()) * self.scale
...
def _band_for_name(name: str, low_band: Tuple[int, int], mid_band: Tuple[int, int], top_band: Tuple[int, int]) -> str:
    ...
    def _in_range(layer_idx: int, band_range: Optional[Tuple[int, int]]) -> bool:
        if band_range is None or layer_idx is None: return False
        s, e = band_range
...
def scale_grads_by_band(grads_tree: Dict[str, mx.array], config: ExperimentConfig) -> Dict[str, mx.array]:
    ...
    for name, g in g_flat:
        ...
        mul = {"low": t_cfg.low_mul, "mid": t_cfg.mid_mul, "top": t_cfg.top_mul, "head": t_cfg.head_mul}.get(band, 1.0)
...
def _maybe_clip_grad_norm(grads_tree: Dict[str, mx.array], max_norm: Optional[float]) -> Tuple[Dict[str, mx.array], float]:
    ...
    try:
        ...
        logger.warning(f"mlx.optim.clip_grad_norm failed: {e}. Falling back to manual clipping.")
        grad_norm = _global_grad_norm(grads_tree)
...
def metal_before_update(num_updates: int, config: ExperimentConfig):
    if not hasattr(config.generation, '_orig_max_gen_len'):
        ...
        setattr(config.trainer, '_orig_num_rollout_samples', config.trainer.num_rollout_samples)

    if num_updates < 32:
        config.data.max_gen_len = min(config.generation._orig_max_gen_len, 160)
        config.max_kv_size = min(config._orig_max_kv_size, 768)
        config.trainer.num_rollout_samples = min(config.trainer._orig_num_rollout_samples, 4)
    else:
...
def _create_4d_attention_mask(tokens: mx.array, pad_token_id: int, dtype: mx.Dtype = TARGET_FLOAT_DTYPE) -> mx.array:
    if tokens.ndim != 2: raise ValueError(f"tokens must be 2D, got {tokens.shape}")
    B, T = tokens.shape
...
def safe_make_sampler(config_or_args: Union[ExperimentConfig, Any], temp: float) -> Callable:
    if isinstance(config_or_args, ExperimentConfig):
        gc = config_or_args.generation
        top_p_val, min_p_val, top_k_val = gc.sampling_top_p, gc.sampling_min_p, gc.sampling_top_k
    else:
        top_p_val = float(getattr(config_or_args, 'sampling_top_p', 0.9))
        min_p_val = float(getattr(config_or_args, 'sampling_min_p', 0.0))
        top_k_val = int(getattr(config_or_args, 'sampling_top_k', 0))

    try:
        return make_sampler(temp=temp, top_p=top_p_val, min_p=min_p_val, top_k=top_k_val)
    except TypeError:
        try: return make_sampler(temp=temp, top_p=top_p_val, min_p=min_p_val)
        except TypeError: return make_sampler(temp=temp, top_p=top_p_val)
        except Exception as e:
            logger.error(f"Failed to create sampler: {e}. Falling back to greedy.", exc_info=True)
            return make_sampler(temp=0.0, top_p=1.0)

def _letter_token_ids(tokenizer: TokenizerWrapper, letters: Sequence[str] = LETTER_ALPH) -> Dict[str, List[int]]:
    out = {}
    for L in letters:
        cand = []
        for suf in ["", " ", ")", ".", " )", " ."]:
...
def _resolve_tag_ids(tokenizer: TokenizerWrapper, gen_config: GenerationConfig) -> Dict[str, Optional[int]]:
    def _one_id(tok_str):
...
def safe_make_sampler(config: ExperimentConfig, temp: float) -> Callable:
    ...
    try:
        return make_sampler(temp=temp, top_p=g_cfg.top_p, min_p=g_cfg.min_p, top_k=g_cfg.top_k)
    except TypeError:
        logger.warning("Falling back to simpler make_sampler signature.")
        return make_sampler(temp=temp, top_p=g_cfg.top_p)

_TOOL_LIKE_MARKERS = ["<tool_call", "</tool_call>", "<tool>", "</tool>", "<tool_", "<function", "</function", "<json", "</json", "<scratchpad", "</scratchpad"]

def _first_token_ids_for_lexemes(tokenizer: TokenizerWrapper, lexemes: Sequence[str]) -> List[int]:
    ids: List[int] = []
    for lx in lexemes:
        if (t := tokenizer.encode(lx, add_special_tokens=False)) and t[0] not in ids: ids.append(t[0])
        if (t_space := tokenizer.encode(" " + lx, add_special_tokens=False)) and t_space[0] not in ids: ids.append(t_space[0])
    return ids
...
def _letter_token_ids(tokenizer: TokenizerWrapper, letters: Sequence[str] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ") -> Dict[str, List[int]]:
    ...
    for L in letters:
        cand = []
        if (ids := tokenizer.encode(L, add_special_tokens=False)) and len(ids) == 1: cand.append(ids[0])
        if (ids := tokenizer.encode(" " + L, add_special_tokens=False)) and len(ids) == 1 and ids[0] not in cand: cand.append(ids[0])
        for suf in [")", ".", " )", " ."]:
            if (ids := tokenizer.encode(L + suf, add_special_tokens=False)) and len(ids) == 1 and ids[0] not in cand: cand.append(ids[0])
        out[L] = cand
    return out
...
def _resolve_tag_ids(tokenizer: TokenizerWrapper, config: ExperimentConfig) -> Dict[str, Optional[int]]:
    ...
    return {"think_start": _one_id(tags.think_start_tag), "think_end": _one_id(tags.think_end_tag), "answer_start": _one_id(tags.answer_start_tag), "answer_end": _one_id(tags.answer_end_tag), "eos": tokenizer.eos_token_id}
...
def make_dynamic_tag_bias_processor(tokenizer: TokenizerWrapper, config: ExperimentConfig, mcq_flags: List[bool]) -> Callable:
    ...
    encourage_ids = _first_token_ids_for_lexemes(tokenizer, gen_cfg.encourage_phrases_for_bias)
    ...
    P_REOPEN_THINK, P_EXTRA_TE = gen_cfg.punish_reopen_think, gen_cfg.punish_extra_think_end
    P_REOPEN_ANS, B_EOS_ANS = gen_cfg.punish_reopen_answer, gen_cfg.bias_eos_after_answer
    ...
    MIN_THINK_TOKENS, B_END_EARLY = gen_cfg.min_think_tokens, gen_cfg.think_end_early_bias
    B_AS_MIN_THINK = gen_cfg.bias_answer_start_after_min_think
    B_ENCOURAGE = gen_cfg.encourage_think_bias
    P_TOOL = gen_cfg.tool_call_penalty * -10.0
    ...
    def _proc_vectorized(hist_list: List[List[int]], logits: mx.array) -> mx.array:
        ...
        history_mx = mx.array([row + [pad_id] * (max_hist_len - len(row)) for row in hist_list], dtype=mx.int32)
        if tool_ids and P_TOOL < 0: logits = logits.at[:, tool_ids].add(P_TOOL)
        ...
        last_ts, last_te, last_as, last_ae = (find_last_pos_mx(t) for t in (ts, te, as_id, ae))
        history_len_mx = mx.array([len(row) for row in hist_list], dtype=mx.int32)

        inside_think = mx.logical_and(last_ts != -1, mx.logical_and(last_te < last_ts, last_as < last_ts))
        ...
        if ts is not None and te is not None:
            ...
            if as_id is not None: logits = logits.at[:, as_id].add(mx.where(last_ae > last_as, P_REOPEN_ANS, 0.0))
            te_count = mx.sum(history_mx == te, axis=1)
            bias_at_te = mx.where(te_count == 0, B_CLOSE, P_EXTRA_TE)
            min_think_penalty_mask = mx.logical_and(inside_think, (k_think < MIN_THINK_TOKENS))
            bias_at_te = mx.where(min_think_penalty_mask, B_END_EARLY, bias_at_te)
            logits = logits.at[:, te].add(bias_at_te)
            can_start_answer = mx.logical_and(last_te > last_as, mx.logical_not(inside_answer))
            min_think_ok = mx.logical_not(B_AS_MIN_THINK)
            if B_AS_MIN_THINK: min_think_ok = (k_think >= MIN_THINK_TOKENS)
            can_start_answer = mx.logical_and(can_start_answer, min_think_ok)
            if as_id is not None: logits = logits.at[:, as_id].add(mx.where(can_start_answer, B_AS, 0.0))

        if eos_tok is not None: logits = logits.at[:, eos_tok].add(mx.where(ae_seen, B_EOS_ANS, 0.0))
        ...
        mcq_first_token_mask = mx.logical_and(is_mcq_mask, mx.logical_and(inside_answer, (k_answer == 0)))
        if mx.any(mcq_first_token_mask).item() and HARD_MASK:
            mcq_allowed_logits = mx.full((V,), neg_inf, dtype=logits.dtype)
            if mcq_letter_ids: mcq_allowed_logits = mcq_allowed_logits.at[mcq_letter_ids].set(LIFT_MCQ)
            if ban_ids: mcq_allowed_logits = mcq_allowed_logits.at[ban_ids].add(BAN_MCQ)
            logits = mx.where(mcq_first_token_mask[:, None], mcq_allowed_logits[None, :], logits)

        non_mcq_first_answer = mx.logical_and(mx.logical_not(is_mcq_mask), mx.logical_and(inside_answer, (k_answer == 0)))
        if mx.any(non_mcq_first_answer).item() and ban_ids: logits = logits.at[non_mcq_first_answer.tolist(), ban_ids].add(BAN_NONMCQ)

        if ae is not None:
            min_ans_len = mx.where(is_mcq_mask, MIN_ANS_MCQ, MIN_ANS)
            min_len_penalty_mask = mx.logical_and(inside_answer, (k_answer < min_ans_len))
            logits = logits.at[:, ae].add(mx.where(min_len_penalty_mask, -8.0, 0.0))
            mcq_close_mask = mx.logical_and(is_mcq_mask, mx.logical_and(inside_answer, (k_answer >= MCQ_CLOSE_K)))
            logits = logits.at[:, ae].add(mx.where(mcq_close_mask, B_MCQ_CLOSE, 0.0))

        return logits
    return _proc_vectorized

def _mask_after_answer(responses_mx: mx.array, initial_mask: mx.array, tokenizer: TokenizerWrapper, config: ExperimentConfig) -> mx.array:
    if responses_mx.ndim != 2: return initial_mask
    ...
    if answer_end_id is None: return initial_mask
    ...
    end_mask = mx.broadcast_to(indices[None, :], responses_mx.shape) < boundary_index[:, None]
