...
def apply_thinking_length_penalty(
    ...
    if thinking_tokens > max_thinking_tokens:
        ...
        logger.debug(f"Thinking penalty: {thinking_tokens} tokens (>{max_thinking_tokens}), penalty={penalty:.3f}")
    else:
        penalty = 0.0
...
def apply_thinking_efficiency_bonus(
    ...
    if bonus > 0:
        logger.debug(f"Efficiency bonus: {thinking_tokens} tokens (optimal={optimal_thinking_tokens}), bonus={bonus:.3f}")
