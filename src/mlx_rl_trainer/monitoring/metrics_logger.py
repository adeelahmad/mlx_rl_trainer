...
from mlx_rl_trainer.utils.text_utils import (
    _preview, _extract_final_numeric, _normalize_ans_for_match,
    extract_think_region, extract_answer_region, _count_words,
    _letters_to_canonical, _indices_to_letters,
)
...
class MetricsLogger:
    ...
    def __init__(self, config: ExperimentConfig, run_id: str):
        ...
        try:
            ...
            logger.error(f"Failed to open metrics CSV at {self.file_path}: {e}", exc_info=True)
            self._file = None
    ...
    def log_metrics(self, metrics: Dict[str, Any], step: int):
        ...
        if self._file is None or self._file.closed:
            if not self._logged_file_closed_warning:
                logger.warning(f"Metrics file '{self.file_path.name}' is closed. Cannot log metrics.")
                self._logged_file_closed_warning = True
            return

        loggable: Dict[str, Union[str, int, float, bool]] = {'update_step': step, 'run_id': self.run_id}
        ...
        if not loggable: return

        with self._lock:
            try:
                # Ensure headers are correct, dynamically add if new metrics appear
                current_headers = ['update_step', 'run_id'] + sorted([k for k in loggable.keys() if k not in ('update_step', 'run_id')])
                ...
                logger.error(f"Error writing metrics CSV to {self.file_path}: {e}", exc_info=True)
        ...
        with self._lock:
            if self._file and not self._file.closed:
                try:
                    ...
                    logger.error(f"Error closing metrics CSV file {self.file_path}: {e}")
...
def _emit_plots_from_csv(csv_path: Path, out_dir: Path):
    ...
    """
    if not csv_path.exists() or csv_path.stat().st_size < 100: # Ensure file exists and has some content
    ...
    try:
        ...
        logger.warning(f"Plot generation skipped: could not read CSV at {csv_path} ({e}).", exc_info=True)
        return
    ...
    if 'update_step' not in df.columns:
        if 'num_updates' in df.columns: df['update_step'] = df['num_updates']
        elif 'step' in df.columns: df['update_step'] = df['step']
        else: df['update_step'] = np.arange(len(df), dtype=int) # Fallback to row index
    ...
    try:
        ...
        def _plot(y_col: str, fname_suffix: str, x_col: str = 'update_step'):
            ...
            if y_col in df.columns and x_col in df.columns and not df.empty:
                ...
                plt.tight_layout()
                plt.savefig(plots_dir / f"{y_col.replace('/', '_').replace('train_', '')}_{fname_suffix}") # Clean filename
                plt.close() # Close figure to free memory

        # Plot core training metrics
        _plot('train/loss', 'loss_vs_updates.png')
        _plot('train/reward_raw_mean', 'reward_mean_vs_updates.png') # Use raw_mean for consistency
        _plot('train/learning_rate', 'lr_vs_updates.png')
        _plot('train/grad_norm', 'grad_norm_vs_updates.png')
        _plot('train/reward_rolling_avg', 'reward_rolling_avg_vs_updates.png') # Plot rolling avg
...
def _maybe_log_samples(
    ...
    *,
    ...
    if config.trainer.log_samples_every <= 0 or update_idx % config.trainer.log_samples_every != 0:
        return

    try:
        global wandb_run
        out_path = Path(config.trainer.sample_log_path) if config.trainer.sample_log_path else config.trainer.output_dir / f"samples_debug_{run_id}.jsonl"
        ...
        reward_tags_config = next((r for r in config.rewards if r.name == "format_structure"), None)
        if reward_tags_config:
            ...
            answer_start_tag = getattr(reward_tags_config, 'answer_open_tag', '<answer>')
            answer_end_tag = getattr(reward_tags_config, 'answer_close_tag', '</answer>')
        ...
        with open(out_path, 'a', encoding='utf-8') as f:
            for i in range(k):
                ...
                if p_idx >= len(prompts_data): continue
                ...
                gen_think_len, gen_answer_len = _extract_think_answer_lengths(generated_text, think_start_tag, think_end_tag, answer_start_tag, answer_end_tag)
                ref_think_len, ref_answer_len = _extract_think_answer_lengths(ref_answer_text, think_start_tag, think_end_tag, answer_start_tag, answer_end_tag)

                # Calculate ratios, handling potential division by zero
                think_ratio = float(gen_think_len) / float(max(ref_think_len, 1)) if ref_think_len > 0 else (1.0 if gen_think_len == 0 else float('inf'))
                answer_ratio = float(gen_answer_len) / float(max(ref_answer_len, 1)) if ref_answer_len > 0 else (1.0 if gen_answer_len == 0 else float('inf'))
                ...
                if think_ratio != float('inf'): all_think_length_ratios.append(think_ratio)
                if answer_ratio != float('inf'): all_answer_length_ratios.append(answer_ratio)

                entry = {
                    'run_id': run_id,
                    'update': update_idx,
                    'is_invalid_batch': is_invalid_batch,
                    'invalid_sample_in_source': original_sample.get('is_invalid_sample', False),
                    'kl_mode': kl_mode,
                    'prompt_preview': _preview(original_sample.get('text', ''), 600) if config.trainer.log_prompts else '[PROMPT REDACTED]',
                    ...
                    'prompt_tokens': int(prompt_token_lens[p_idx]) if p_idx < len(prompt_token_lens) else None,
                    'response_tokens': int(response_token_lens[i]) if i < len(response_token_lens) else None,
                    ...
                    'think_length_ratio': think_ratio if think_ratio != float('inf') else None,
                    'answer_length_ratio': answer_ratio if answer_ratio != float('inf') else None,
                    'ref_answer_preview': _preview(ref_answer_text, 300),
                    'mcq_ref_letter': ref_letters_list[i] if ref_letters_list and i < len(ref_letters_list) else '',
                    'mcq_gen_letter': gen_letters_list[i] if gen_letters_list and i < len(gen_letters_list) else '',
                    'is_mcq': bool(is_mcq_list[i]) if is_mcq_list and i < len(is_mcq_list) else original_sample.get('is_mcq', False),
        ...
        if wandb_run is not None and config.monitoring.use_wandb:
            try:
                ...
                wandb_metrics = {
                    'samples/reward_total_mean': float(np.mean(rewards_total[:k_actual])),
                    'samples/reward_total_std': float(np.std(rewards_total[:k_actual])),
                    'samples/reward_format_mean': float(np.mean(rewards_fmt[:k_actual])),
                    'samples/reward_content_mean': float(np.mean(rewards_cont[:k_actual])),

                    'samples/gen_think_length_mean': float(np.mean(all_gen_think_lens)) if all_gen_think_lens else 0,
                    'samples/gen_think_length_std': float(np.std(all_gen_think_lens)) if all_gen_think_lens else 0,
                    'samples/gen_think_length_min': int(np.min(all_gen_think_lens)) if all_gen_think_lens else 0,
                    'samples/gen_think_length_max': int(np.max(all_gen_think_lens)) if all_gen_think_lens else 0,

                    'samples/ref_think_length_mean': float(np.mean(all_ref_think_lens)) if all_ref_think_lens else 0,
                    'samples/ref_answer_length_mean': float(np.mean(all_ref_answer_lens)) if all_ref_answer_lens else 0,
                    'samples/think_length_ratio_mean': float(np.mean(all_think_length_ratios)) if all_think_length_ratios else 0,
                    'samples/answer_length_ratio_mean': float(np.mean(all_answer_length_ratios)) if all_answer_length_ratios else 0,

                    'samples/response_tokens_mean': float(np.mean(response_token_lens[:k_actual])),

                    'samples/excessive_thinking_rate': float(np.mean([1.0 if x > config.trainer.think_len_max else 0.0 for x in all_gen_think_lens])),
                    'samples/too_brief_thinking_rate': float(np.mean([1.0 if x < config.trainer.min_think_tokens else 0.0 for x in all_gen_think_lens])),

                    'samples/mcq_accuracy': _calculate_mcq_accuracy(ref_letters_list, gen_letters_list, is_mcq_list, k_actual) if ref_letters_list and gen_letters_list else None,
                ...
                logger.warning(f"Failed to log sample summary to wandb: {e}", exc_info=True)
