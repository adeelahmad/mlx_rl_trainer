...
import logging, csv, json, threading
...
def _calculate_mcq_accuracy(refs: Optional[List[str]], gens: Optional[List[str]], is_mcq: Optional[List[bool]], k: int) -> float:
    if not all((refs, gens, is_mcq)) or k == 0: return 0.0
    correct, total = 0, 0
    for i in range(k):
        if is_mcq[i]:
            total += 1
            if refs[i] == gens[i] and refs[i]: correct += 1
...
class MetricsLogger:
    ...
    def log_metrics(self, metrics: Dict[str, Any], step: int):
        if not self._file or self._file.closed: return
        ...
        with self._lock:
            try:
                ...
                if self._writer is None or self._headers != current_headers:
                    ...
                    if is_empty: self._writer.writeheader()
    ...
    def close(self):
        with self._lock:
            if self._file and not self._file.closed: self._file.close(); self._file = None

def _emit_plots_from_csv(csv_path: Path, out_dir: Path):
    if not (PANDAS_AVAILABLE and MPL_AVAILABLE) or not csv_path.exists() or csv_path.stat().st_size < 100: return
    try:
        ...
        def _plot(y_col: str, fname_suffix: str, x_col: str = "update_step"):
            if y_col in df.columns:
                ...
                plt.xlabel(x_col.replace("_", " ").title()); plt.ylabel(y_col.replace("_", " ").title())
                plt.title(f"{y_col.replace('_', ' ').title()} vs {x_col.replace('_', ' ').title()}")
                plt.grid(True, alpha=0.5); plt.tight_layout()
                plt.savefig(plots_dir / f"{y_col.replace('/', '_')}_{fname_suffix}.png"); plt.close()

        plot_map = { "train/loss": "loss", "train/reward_raw_mean": "reward", "train/learning_rate": "lr", "train/grad_norm": "grad_norm"}
        for col, name in plot_map.items(): _plot(col, name)
...
def _maybe_log_samples(config: ExperimentConfig, update_idx: int, prompts_data: List[Dict], decoded_responses: List[str], rewards_data: Dict, kl_mode: str, run_id: str, is_invalid_batch: bool):
    if config.monitoring.log_samples_every <= 0 or update_idx % config.monitoring.log_samples_every != 0: return
    ...
    try:
        ...
        with open(out_path, "a", encoding="utf-8") as f:
            for i in range(k):
                ...
                entry = {
                    "update": update_idx, "is_invalid_batch": is_invalid_batch, "kl_mode": kl_mode,
                    "prompt": _preview(original_sample.get("text",""), 600) if config.monitoring.log_prompts else "[REDACTED]",
                    "generated": _preview(gen_text, 600), "reference": _preview(ref_text, 300),
                    "reward_total": rewards_data["total"][i], "gen_think_len": gen_think_len, "gen_ans_len": gen_ans_len,
                    "ref_think_len": ref_think_len, "ref_ans_len": ref_ans_len, "ts": time.strftime("%Y-%m-%d %H:%M:%S")
                }
                for r_name, r_vals in rewards_data.items():
                    if r_name != "total": entry[f"reward_{r_name}"] = r_vals[i]
