...
from mlx_rl_trainer.utils.mlx_utils import (
    _create_4d_attention_mask, safe_make_sampler, _resolve_tag_ids, 
    _first_token_ids_for_lexemes, _letter_token_ids, make_dynamic_tag_bias_processor, _mask_after_answer
)
...
def generate_rollouts_for_batch(
    ...
    prompts_data: List[Dict], dataset: "Dataset", config: ExperimentConfig, reward_composer: RewardComposer,
    ...
    _, prompts_mx, max_prompt_len = build_rollout_batch(tokenizer, dataset, indices, config)
    ...
    if prompts_mx.size == 0: return {}, 0.0, {}
    ...
    for step in range(max_gen_len):
        ...
        ended_prev = ended
        if eos_id is not None: ended = mx.logical_or(ended, sampled_tokens == eos_id)
    ...
    contexts = [reward_composer.context_cls(
        generated_text=decoded[i], prompt_text=prompts_data_replicated[i]['text'],
    ...
    advantages = grpo_algo.compute_advantages(rewards_total, config.trainer.num_rollout_samples)
    ...
    ref_log_probs = mx.take_along_axis(ref_log_probs_all, responses_mx[..., None].astype(mx.int64), axis=-1).squeeze(-1)
    ...
    rollout_batch = {
        "tokens": full_seq, "response_mask": response_mask, "advantages": advantages,
        "ref_log_probs": ref_log_probs, "actor_log_probs": actor_log_probs,
    }
    ...
    model.train()
    if ref_model: ref_model.train() # Set back to train mode if it was changed
    gc.collect()
    mx.clear_cache()
