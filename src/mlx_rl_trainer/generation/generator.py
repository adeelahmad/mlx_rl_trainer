...
def _create_thinking_answer_masks(
    ...
    thinking_lengths, answer_lengths = [], []
    (
        missing_answer_count,
        missing_thinking_count,
        truncated_count,
        empty_think_in_answer_count,
    ) = (0, 0, 0, 0)
    ...
    for batch_idx in range(batch_size):
        ...
        if think_end_pos == -1:
            ...
            (
                accumulated_len,
                opening_tag_end_token,
                thinking_content_start_token,
                thinking_content_end_token,
                closing_tag_end_token,
            ) = (0, 0, 0, 0, 0)
            ...
            if closing_tag_end_token == 0:
                ...
                thinking_content_start_token, thinking_content_end_token = (
                    opening_tag_end_token,
                    closing_tag_end_token,
                )
    ...
    logger.debug(
        f"Masks: thinking={stats['generation/thinking_tokens_avg']:.1f}, answer={stats['generation/answer_tokens_avg']:.1f}, ratio={stats['generation/thinking_answer_ratio']:.2f}:1"
    )
...
def generate_rollouts_for_batch(
    ...
    num_prompts = len(prompts_data)  # <--- It is defined here!
    ...
    if getattr(config.trainer, "use_dual_gradients", False):
        try:
            ...
            if mx.sum(thinking_mask).item() == 0 and mx.sum(answer_mask).item() == 0:
                logger.error("Both thinking and answer masks are empty!")
                thinking_mask, answer_mask, mask_stats = None, None, {}
    ...
    if thinking_mask is not None and answer_mask is not None:
        rollout_batch["thinking_mask"], rollout_batch["answer_mask"] = (
            thinking_mask,
            answer_mask,
        )

    if getattr(config.trainer, "use_sft_on_answer", False):
        try:
            ...
            for prompt_data in prompts_data_replicated:
                ...
                if "prompt_len" in prompt_data:
                    ref_resp = tokenizer.encode(ref_text)[prompt_data["prompt_len"] :]
                elif "text" in prompt_data:
                    prompt_toks, ref_full = tokenizer.encode(
                        prompt_data["text"]
                    ), tokenizer.encode(ref_text)
                    ref_resp = ref_full[len(prompt_toks) :]
                else:
                    ref_resp = tokenizer.encode(ref_text)

                padded = (
                    ref_resp[:max_resp_len]
                    if len(ref_resp) > max_resp_len
                    else ref_resp + [pad_id] * (max_resp_len - len(ref_resp))
                )
                reference_tokens_padded.append(padded)

            rollout_batch["reference_tokens"] = mx.array(
                reference_tokens_padded, dtype=mx.int32
            )
            del reference_tokens_padded
        except Exception as e:
            logger.error(f"Failed to add reference tokens for SFT: {e}", exc_info=True)
