...
class PagedKVCache:
    ...
    def __init__(
        ...
        mx.eval(self.key_cache, self.value_cache) # Eagerly evaluate to allocate GPU memory
    ...
    def reset(self):
        ...
        logger.debug('[PagedKVCache] Cache state has been reset and all blocks are free.')

    def _allocate_block(self) -> int:
        """Pops a block index from the free pool."""
        if not self.free_blocks:
            raise MemoryError('PagedKVCache is out of memory blocks. Increase `num_blocks` or reduce batch size.')
    ...
    def allocate_sequence(self, sequence_id: int, num_tokens: int):
        ...
        if sequence_id in self.sequence_map:
            raise ValueError(f"Sequence ID {sequence_id} is already allocated. Free it first.")
        ...
        allocated_block_indices = [self._allocate_block() for _ in range(num_required_blocks)]
        self.sequence_map[sequence_id] = allocated_block_indices
        self.sequence_lengths[sequence_id] = num_tokens
        logger.debug(f"Allocated {num_required_blocks} blocks {allocated_block_indices} for sequence {sequence_id} (initial length: {num_tokens}).")
    ...
    def free_sequence(self, sequence_id: int):
        ...
        if sequence_id not in self.sequence_map:
            logger.debug(f"Attempted to free unknown sequence ID {sequence_id}. No action taken.")
            return
    ...
    def append_token(self, sequence_id: int):
        ...
        """
        if sequence_id not in self.sequence_map:
            logger.warning(f"Appending to unallocated sequence ID {sequence_id}. Allocating new minimal sequence (1 block).")
            self.allocate_sequence(sequence_id, 1) # Allocate a block for this new sequence
            return
        ...
        if current_logical_len + 1 > current_allocated_blocks * self.block_size:
            logger.debug(f"Sequence {sequence_id} at logical length {current_logical_len} needs a new block ({current_allocated_blocks} blocks currently allocated).")
            new_block_idx = self._allocate_block() # May raise MemoryError if no blocks are free
            self.sequence_map[sequence_id].append(new_block_idx)
            logger.debug(f"Allocated new block {new_block_idx} for sequence {sequence_id}.")

        self.sequence_lengths[sequence_id] += 1
        logger.debug(f"Sequence {sequence_id} logical length increased to {self.sequence_lengths[sequence_id]}.")
