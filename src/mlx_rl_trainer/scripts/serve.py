...
)  # For sampling and attention mask
...
class GenerateRequest(BaseModel):
    prompt: str = Field(
        ..., min_length=1, description="The input prompt for text generation."
    )
    max_tokens: int = Field(
        128, gt=0, description="Maximum number of tokens to generate."
    )
    ...
    stop_sequences: Optional[List[str]] = Field(
        None, description="List of sequences to stop generation."
    )
    stream: bool = Field(False, description="Whether to stream response tokens.")
    system_prompt: Optional[str] = Field(
        None, description="Optional system prompt to apply."
    )
...
async def _perform_generation(request: GenerateRequest) -> Tuple[str, int, float]:
    ...
    system_prompt_to_use = (
        ...
        else global_exp_config.system_prompt
    )
    ...
    # Setup KV cache
    max_kv_size = global_exp_config.max_kv_size or (
        prompt_mx.shape[1] + request.max_tokens
    )
    caches = cache.make_prompt_cache(
        global_model, max_kv_size=max_kv_size
    )  # Using mlx_lm's standard TreeCache

    global_model.eval()

    # First forward pass
    attn_mask = _create_4d_attention_mask(
        prompt_mx, global_tokenizer.pad_token_id, dtype=mx.bfloat16
    )  # From mlx_utils
    ...
    logits_processors = make_logits_processors(
        repetition_penalty=global_exp_config.trainer.repetition_penalty
        or 1.1,  # Use config value
        repetition_context_size=global_exp_config.trainer.repetition_context_size
        or 20,  # Use config value
...
async def generate_text(request: GenerateRequest):
    ...
    """
    try:
        generated_text, tokens_generated, time_taken_ms = await _perform_generation(
            request
        )
...
async def health_check():
    ...
    status = (
        "ok" if global_model is not None and global_tokenizer is not None else "loading"
    )
    return {
        "status": status,
...
async def startup_event():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the ExperimentConfig YAML.",
    )
    ...
    try:
        ...
        global_model, global_tokenizer = model_manager.load_model(
            Path(cli_args.model_path), "serving_model"
        )
        ...
        if global_exp_config.use_paged_kv_cache:
            ...
            )  # Assuming MockModel stores config
            ...
            global_paged_kv_cache = PagedKVCache(
                ...
                head_dim=model_arch_config.get("hidden_size", 128)
                // model_arch_config.get("num_attention_heads", 4),
        ...
        logger.critical(
            f"Failed to load model/tokenizer/config during startup: {e}", exc_info=True
        )
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    from mlx_rl_trainer.utils.mlx_utils import (
        _create_4d_attention_mask as _create_attn_mask_for_serve,
    )
