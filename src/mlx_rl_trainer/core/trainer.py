...
class TrainingMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        data = { "loss": self.loss, "reward_mean": self.reward_mean, "grad_norm": self.grad_norm,
                 "learning_rate": self.learning_rate, "step_time_s": self.step_time_s }
...
class BaseTrainer(ABC):
    def __init__(self, config: ExperimentConfig, model_manager: Any, data_manager: Any, checkpoint_manager: Any):
        self.config = config
    ...
    @abstractmethod
    def generate_rollouts(self, batch_prompts_data: Dict[str, List[Any]], update_step: int) -> Tuple[Dict[str, mx.array], float, Dict[str, float]]:
        """
    ...
    def run(self) -> None:
        ...
        """
        from tqdm import trange # Import tqdm here to avoid circular dependency with logging setup

        self.global_step, self.current_epoch = self._setup()
        logging.info(f"Training will start from update step {self.global_step}, epoch {self.current_epoch}.")

        pbar = trange(self.global_step, self.config.trainer.num_training_steps,
                      initial=self.global_step, desc="Training Progress", unit="update")
        ...
        try:
            ...
            for update_step in pbar:
                ...
                for _ in range(self.config.trainer.grad_accum_steps):
                    ...
                    if prompts_batch is None:
                        ...
                        logging.info(f"Epoch finished. Starting new epoch: {self.current_epoch}.")
                        train_data_iterator = self.data_manager.get_dataloader('train', self.config.trainer.ppo_batch_size)
                        prompts_batch = next(train_data_iterator, None)
                        if prompts_batch is None:
                            raise TrainingRuntimeError("Data stream exhausted, cannot get next batch.")

                    # Generate rollouts and perform one training step
                    rollout_data, avg_reward_from_rollouts, raw_rewards_breakdown = self.generate_rollouts(prompts_batch, self.global_step)
                ...
                avg_loss = sum(m.loss for m in accumulated_metrics) / len(accumulated_metrics)
                avg_reward_mean = sum(m.reward_mean for m in accumulated_metrics) / len(accumulated_metrics)
                avg_grad_norm = sum(m.grad_norm for m in accumulated_metrics) / len(accumulated_metrics)
                ...
                is_eval_step = (update_step > 0 and update_step % self.config.trainer.eval_every == 0)
                is_save_step = (update_step > 0 and update_step % self.config.trainer.save_every == 0)

                eval_metric_for_checkpoint = -float('inf')
                if is_eval_step:
                    logging.info(f"Starting evaluation at update step {update_step}...")
                    eval_results = self.evaluate(update_step) # This calls configured evaluators

                    # Log evaluation results (assuming metrics_logger is available)
                    from mlx_rl_trainer.monitoring.metrics_logger import MetricsLogger # Local import for logging
                    metrics_logger_instance = MetricsLogger(self.config, run_id=str(uuid.uuid4())) # Mock run_id for now
                    for eval_metric in eval_results:
                        metrics_logger_instance.log_metrics(eval_metric.to_dict(), step=update_step)
                        # Identify primary metric for checkpointing (e.g., pass@1 for HumanEval)
                        if eval_metric.task_name == "human_eval" and "pass@1" in eval_metric.additional_info:
                        ...
                        elif eval_metric.task_name == "gsm8k" and "bench/acc" in eval_metric.additional_info:
                ...
                if is_save_step or (is_eval_step and eval_metric_for_checkpoint > -float('inf')):
                    self.checkpoint_manager.save_checkpoint(
                        step=update_step,
                        model_state=dict(mx.utils.tree_flatten(self.actor_model.parameters())),
                        optimizer_state=self.optimizer.state if self.optimizer else {},
                        metadata={'num_updates': update_step, 'epoch': self.current_epoch},
            ...
            final_metric = pbar.postfix.get("Reward", 0.0) if hasattr(pbar, 'postfix') and "Reward" in pbar.postfix else 0.0
            try:
                self.checkpoint_manager.save_checkpoint(
                    step=pbar.n,
                    model_state=dict(mx.utils.tree_flatten(self.actor_model.parameters())),
                    optimizer_state=self.optimizer.state if self.optimizer else {},
                    metadata={'num_updates': pbar.n, 'epoch': self.current_epoch, 'final_run': True},
