...
try:
    ...
    class MLXLoRALinear:
        pass  # Dummy for type checking if mlx_lm is not fully installed
...
class CheckpointManager:
    ...
    def _load_existing_checkpoints(self):
        ...
        for p in self.output_dir.iterdir():
            if p.is_dir() and (p / "metadata.json").is_file():
                match = checkpoint_regex.match(p.name)
                if match:
    ...
    def save_checkpoint(
        ...
        # Fix #8: Correct type hint for optimizer
        ...
        try:
            ...
            # Fix #12: Ensure save_optimizer_state is in metadata and respected
    ...
    def load_latest_state(
        ...
        chosen_path = self.resume_from_path or (self.output_dir / "latest").resolve()
        if not chosen_path.exists():
            rprint(
                "[yellow]No checkpoint found to resume from. Starting from scratch.[/yellow]"
            )
            return 0, {}
