...
try:
    ...
    logging.warning(
        "mlx-lm not found. CheckpointManager will have limited LoRA handling."
    )

    # Mock minimal functions if mlx_lm is not available
    def remove_lora_layers(model: Any):
        return model

    def fuse_lora_layers(model: Any):
        return model

    def mlx_lm_load(path: Path):
        raise CheckpointError("mlx-lm not available for loading model.")
...
class CheckpointManager:
    ...
    def __init__(self, output_dir: Path, keep_last_n: int = 3, save_best: bool = True):
        ...
        self.best_metric: float = -float(
            "inf"
        )  # Tracks the best metric seen for saving 'best' model
        ...
        logging.info(
            f"CheckpointManager initialized for {output_dir}. Keeping last {keep_last_n} checkpoints."
        )
    ...
    def _load_existing_checkpoints(self):
        ...
        found = [
            p
            for p in self.output_dir.glob("checkpoint_step_*")
        ...
        if best_symlink.is_symlink() and best_symlink.resolve().is_dir():
            try:
                with open(best_symlink.resolve() / "metadata.json", "r") as f:
                    ...
                    rprint(
                        f"Restored best metric from 'best' checkpoint: {self.best_metric:.4f}."
                    )
            except Exception as e:
                logging.warning(
                    f"Could not load best_metric from 'best' checkpoint symlink: {e}"
                )
    ...
    def save_checkpoint(
        ...
        temp_path = (
            self.output_dir / f".{checkpoint_name}.tmp"
        )  # Temporary path for atomic save
        ...
        try:
            ...
            mlx_rng_key = mx.random.key(
                random.randint(0, 2**31 - 1)
            )  # Generate a new key/seed for storage
            rng_state = {
                "python": random.getstate(),
                "numpy": np.random.get_state(),
                "mlx_key_seed": int(
                    mlx_rng_key[0].item()
                ),  # Store first element of MLX key for reproducibility, or more if needed
            ...
            with open(temp_path / "metadata.json", "w") as f:
                json.dump(
                    metadata, f, indent=2, default=str
                )  # default=str handles Path/mx.array types
            ...
            logging.critical(
                f"Failed to save checkpoint at step {step}: {e}", exc_info=True
            )
            if temp_path.exists():
                shutil.rmtree(temp_path)  # Clean up failed temp directory
            raise CheckpointError(f"Atomic save failed for step {step}.") from e

    def load_latest_state(
        self, model: nn.Module, optimizer: Any
        ...
        if not self._checkpoints:
            rprint(
                "[yellow]No checkpoints found. Starting from step 0, epoch 0.[/yellow]"
            )
        ...
        try:
            ...
            )  # Ensure all parameters are evaluated
            ...
            if rng_state:
                ...
                if "mlx_key_seed" in rng_state and isinstance(
                    ...
                    mx.random.seed(
                        random.randint(0, 2**31 - 1)
                    )  # Random seed if no specific MLX RNG state saved
            ...
            logging.critical(
                f"Failed to load checkpoint {latest_path.name}: {e}", exc_info=True
            )
            raise CheckpointError(
                f"Failed to load state from {latest_path.name}."
            ) from e
    ...
    def _rotate_checkpoints(self):
        ...
        self._checkpoints = [
            chk for chk in self._checkpoints if chk.exists()
        ]  # Clean up any broken entries
