...
class CheckpointConfig(BaseModel):
    ...
    save_every: PositiveInt = Field(
        12, description="Save a full checkpoint every N training updates."
    )
...
class TrainerParams(BaseModel):
    ...
    reward_smoothing_window: PositiveInt = Field(50)

    # Add the missing field
    effective_batch_size: int = Field(0, exclude=True)

    use_dual_gradients: bool = Field(True)
    thinking_layer_start: Optional[int] = Field(16)
    thinking_layer_end: Optional[int] = Field(26)
    ...
    adaptive_gradient_weights: bool = Field(False)
    ...
    thinking_penalty_rate: Optional[NonNegativeFloat] = Field(0.15)
...
class ExperimentConfig(BaseModel):
    ...
    sft_thinking_weight:NonNegativeFloat = Field(0.3)      # 10% SFT on thinking layers
    sft_answer_weight: NonNegativeFloat =  Field(0.7)        # 100% SFT on answer layers
