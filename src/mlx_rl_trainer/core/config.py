...
class CheckpointConfig(BaseModel):
    save_dir: Path = Field(Path("./checkpoints"), description="Directory relative to output_dir to save checkpoints.")
    save_every: PositiveInt = Field(10, description="Save a checkpoint every N training updates.")
    keep_last_n: PositiveInt = Field(3, description="Number of most recent checkpoints to retain.")
    save_optimizer_state: bool = Field(False, description="Whether to save the optimizer's state.")
    save_best: bool = Field(True, description="Save a copy of the best performing checkpoint.")
    best_model_metric: str = Field("validation/reward_mean", description="Metric to use for determining the 'best' model checkpoint.")
    checkpoint_save_retries: int = Field(3, ge=0, description="Number of retries if checkpoint saving fails.")
    checkpoint_save_backoff_factor: float = Field(5.0, ge=0, description="Backoff factor in seconds for retries.")


class CheckpointConfigX(BaseModel):
    save_dir: Path = Field(
        Path("./checkpoints"),
...
class TrainerParams(BaseModel):
    ...
    thinking_layer_end: Optional[int] = Field(28)
    answer_layer_start: Optional[int] = Field(22)
    answer_layer_end: Optional[int] = Field(36)
    answer_gradient_weight: Optional[NonNegativeFloat] = Field(3.2)
    ...
    use_thinking_bonus: bool = Field(True)
    efficiency_bonus_weight: Optional[NonNegativeFloat] = Field(0.2)
...
COMPRESS EVERYTHING. Every word must earn its place."""
    ...
    sft_weight: NonNegativeFloat = Field(0.4)
    sft_thinking_weight: NonNegativeFloat = Field(0.6)  # 10% SFT on thinking layers
    sft_answer_weight: NonNegativeFloat = Field(1.0)  # 100% SFT on answer layers
