...
class GenerationConfig(BaseModel):
    ...
    think_temperature: NonNegativeFloat = Field(0.4)
    answer_temperature: NonNegativeFloat = Field(0.5)
...
class TrainerParams(BaseModel):
    ...
    reward_smoothing_window: PositiveInt = Field(500)
...
COMPRESS EVERYTHING. Every word must earn its place."""
    ...
    encourage_phrases_for_bias: List[str] = Field(
        default_factory=lambda: [
            ...
            "filtering",
            "retrieval",
            "I conclude that its undeterminable.<\think>\\n",
        ]
    )
