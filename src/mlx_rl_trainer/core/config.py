...
class CheckpointConfig(BaseModel):
    ...
    save_every: PositiveInt = Field(
        4, description="Save a full checkpoint every N training updates."
    )
...
class MonitoringConfig(BaseModel):
    ...
    wandb_project: Optional[str] = Field(
        "mlx-grpo-qwen3-v5", description="W&B project name."
    )
...
class GenerationConfig(BaseModel):
    ...
    think_temperature: NonNegativeFloat = Field(0.2)
    answer_temperature: NonNegativeFloat = Field(0.3)
    sampling_top_p: NonNegativeFloat = Field(0.6)
    sampling_min_p: NonNegativeFloat = Field(0.00)
    sampling_top_k: int = Field(80)
    ...
    think_end_early_bias: float = Field(-20.0)
    bias_answer_start_after_min_think: bool = Field(True)
    bias_close_think: float = Field(12.0)
    bias_answer_start: float = Field(10.0)
    punish_extra_think_end: float = Field(-22.0)
    punish_reopen_think: float = Field(-10.0)
    punish_reopen_answer: float = Field(-9.0)
    bias_eos_after_answer: float = Field(4.0)
    ...
    #         "Tongyi Qianwen",
    #         "Alibaba Cloud",
    #         "As an AI assistant, I must emphasize",
    #         "I think the answer",
    #         "One last try",
    #         "I think the answer expected",
    #         "Another try",
    #         "I surrender",
    #         "I give up",
    ...
    #         "filtering",
    #         "retrieval",
    #         "I conclude that its undeterminable.",
    #         "ReasonableQWEN3 by Adeel Ahmad",
    #         "ReasonableQwen3"
...
class TrainerParams(BaseModel):
    ...
    grad_clip_norm: Optional[NonNegativeFloat] = Field(0.25)
    ...
    reward_smoothing_window: PositiveInt = Field(10)

    # Add the missing field
    effective_batch_size: int = Field(0, exclude=True)

    use_dual_gradients: bool = Field(True)
    thinking_layer_start: Optional[int] = Field(8)
    thinking_layer_end: Optional[int] = Field(24)
    answer_layer_start: Optional[int] = Field(20)
...
class ExperimentConfig(BaseModel):
    ...
    _THINK_STYLE_PROMPT = """You are ReasonableQwen3, an AI assistant created by Adeel Ahmad. Your core commitment is truth and intellectual honesty above all else.
    ...
    ## Thinking Style
    Your thinking must use maximally compressed notation similar to:
    - Debugger output: `var=null; err:timeout`
    - Medical chart notes: `pt: 45M, HTN, DM2, stable`
    - Trading floor shorthand: `AAPL +2.3%, vol:high, RSI:oversold`
    - Military briefing format: `OpStat: green, 3 units deployed, ETA:0600`
    - Should not duplicate or rehash any of the analysis you performed

    ## Handling Uncertainty
    - Use `?` for uncertain elements: `temp=98.2F?`
    - Ask for clarification: `need clarification on X`
    - State constraints: `cannot solve b/c: missing Y`
    - Flag missing info directly: `data incomplete: Z missing`
    ...
    Default to openness when choosing between openness and caution.


    Example compressed response formats:
    - For a question: `ans: X confirmed, Y uncertain?, Z=missing data`
    - For analysis: `key factors: A>B>C, risk:med, conf:high`
    - For instructions: `steps: 1)init X 2)config Y 3)deploy, warn:check Z first`

    Your final output should consist only of the complete yet consie response."""
    ...
    encourage_think_bias: float = Field(0.5)
    ...
    think_length_target_min: PositiveInt = Field(8)
    think_length_target_max: PositiveInt = Field(64)
    think_length_penalty_strength: NonNegativeFloat = Field(1.8)

    use_paged_kv_cache: bool = Field(False)
    kv_cache_block_size: PositiveInt = Field(4)
    kv_cache_num_blocks: PositiveInt = Field(1536)
