...
class DataConfig(BaseModel):
    ...
    max_prompt_len: PositiveInt = 350
    max_gen_len: PositiveInt = 96
...
class ModelConfig(BaseModel):
    ...
    def set_default_ref_model_path(self) -> "ModelConfig":
        if self.ref_model_path is None: self.ref_model_path = self.model_path
        return self

class CheckpointConfig(BaseModel):
    save_dir: Path = Path("checkpoints")
    save_every: PositiveInt = 10
    keep_last_n: PositiveInt = 3
    save_optimizer_state: bool = False

class MonitoringConfig(BaseModel):
    use_wandb: bool = True
    wandb_project: Optional[str] = "mlx-grpo"
    wandb_entity: Optional[str] = None
    wandb_run_name: Optional[str] = None
    log_samples_every: PositiveInt = 1
    max_logged_samples: PositiveInt = 50
...
class GenerationConfig(BaseModel):
    ...
    repetition_penalty: Optional[float] = Field(1.15, ge=1.0)
...
class TrainerParams(BaseModel):
    ...
    num_training_steps: PositiveInt = 45869
    learning_rate: NonNegativeFloat = 2e-6
    ppo_batch_size: PositiveInt = 1
    num_rollout_samples: PositiveInt = 2
    grad_accum_steps: PositiveInt = 2
    grpo_beta: NonNegativeFloat = 0.025
    seed: int = 432
    ...
    grad_checkpoint_layers: PositiveInt = 0
    low_band: Tuple[int, int] = (0, 15)
    mid_band: Tuple[int, int] = (16, 23)
    top_band: Tuple[int, int] = (24, 35)
    low_mul: NonNegativeFloat = 0.1
    mid_mul: NonNegativeFloat = 0.95
    top_mul: NonNegativeFloat = 1.5
    head_mul: NonNegativeFloat = 1.2
    train_layer_start: Optional[int] = 26
    train_layer_end: Optional[int] = 35
    think_length_target_min: PositiveInt = 32
    think_length_target_max: PositiveInt = 128
    think_length_penalty_strength: NonNegativeFloat = 0.15
    think_length_penalty_type: str = "quadratic"
    enable_think_length_penalty: bool = True
    use_custom_batch_builder: bool = False
    invalid_sample_layers: str = "33,34,35"
    invalid_sample_frequency: PositiveInt = 2
    invalid_sample_layers_set: Set[int] = Field(default_factory=set, exclude=True)
    eval_every: PositiveInt = 10
    reward_smoothing_window: PositiveInt = 20
    effective_batch_size: PositiveInt = Field(1, init=False) # FIX: Remove `exclude=True`
    current_update: int = Field(0, init=False)
    ...
    def populate_derived_fields(self) -> "TrainerParams":
        ...
        if isinstance(self.invalid_sample_layers, str):
            ...
            except ValueError: self.invalid_sample_layers_set = set()
...
class ExperimentConfig(BaseModel):
    ...
    kv_cache_block_size: PositiveInt = 16
    kv_cache_num_blocks: PositiveInt = 2048
    allow_cross_arch_ref: bool = False
    align_bridge_path: Optional[Path] = None
    align_bridge_weight: NonNegativeFloat = 1.0
    align_pool: Literal["mean", "last"] = "mean"
    align_after_tag: str = "</think>"
    ...
    def validate_reward_weights_sum(self) -> "ExperimentConfig":
        if self.rewards:
            ...
            if not (0.99 <= total_weight <= 1.01):
                logger.warning(f"Reward weights do not sum to 1.0 (got {total_weight:.2f}).")
        return self
