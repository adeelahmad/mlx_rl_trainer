...
from pydantic import BaseModel, Field, PositiveInt, NonNegativeFloat, ValidationError, model_validator, ConfigDict
...
DEFAULT_ABBREVIATIONS = [
    "w/", "w/o", "b/c", "re:", "vs", "via", "per", "thru",
    "i.e.", "e.g.", "etc.", "cf", "viz", "NB",
    ...
    "rev", "exp", "proj", "KPI", "ROI", "YoY", "MoM",
    "cust", "mkt", "comp", "strat", "ops",
...
DEFAULT_BAN_KEYWORDS = [
    ...
    "Confused", "stuck", "frustrated", "Uncertain", "Unclear", "I'm guessing",
    "maybe the answer is", "I'm not sure", "Probably", "Perhaps", "Possibly",
    "Circular reasoning", "In some way", "Magically", "For some reason",
    "Too complicated", "It just works", "Something is off",
    "Wait, but", "Wait, maybe", "Wait, actually", "Hold on", "another thought:",
    "Alternatively", "Actually", "Or maybe",
    "Flowery language, hedging, or conversational filler",
    "Furthermore", "Moreover", "Nevertheless", "Nonetheless", "Subsequently",
    "Therefore, it can be concluded", "In conclusion", "To summarize",
...
class DataConfig(BaseModel):
    ...
    max_prompt_len: PositiveInt = Field(
        512, description="Maximum token length for input prompts. Prompts will be truncated if longer."
    )
    ...
    shuffle_data: bool = Field(
        True, description="Whether to shuffle training data at the beginning of each epoch."
    )

    dataset_prompt_key: str = Field(
        "prompt", description="Key in the raw dataset dictionary that holds the prompt text."
    )
    dataset_answer_key: str = Field(
        "completion", description="Key in the raw dataset dictionary that holds the reference answer/completion."
    )
...
class ModelConfig(BaseModel):
    ...
    lora_scale_by_rank: bool = Field(
        True, description="Whether to scale LoRA weights by rank for better performance."
    )
...
class CheckpointConfig(BaseModel):
    ...
    save_dir: Path = Field(
        "checkpoints", description="Directory relative to `output_dir` to save checkpoints."
    )
    save_every: PositiveInt = Field(
        100, description="Save a full checkpoint every N training updates."
    )
    keep_best_n: PositiveInt = Field(
        3, description="Number of most recent checkpoints to retain (older ones are deleted, excluding the 'best')."
    )
    save_optimizer_state: bool = Field(
        False, description="Whether to save the optimizer's state along with model weights (increases checkpoint size)."
    )

class MonitoringConfig(BaseModel):
    """Configuration for monitoring, logging, and visualization."""
    use_wandb: bool = Field(
        True, description="Enable Weights & Biases (W&B) logging for experiment tracking."
    )
    ...
    log_samples_every: PositiveInt = Field(
        10, description="Log generated text samples to W&B and a local NDJSON file every N updates."
    )
    ...
    sample_log_path: Optional[Path] = Field(
        None, description="Custom path to save NDJSON sample logs. Defaults to `output_dir/samples_debug_*.jsonl`."
    )
...
class TrainerParams(BaseModel):
    ...
    output_dir: Path = Field(
        Path("./outputs"), description="Base directory for all training outputs (checkpoints, logs, plots)."
    )
    num_training_steps: PositiveInt = Field(10000, description="Total number of training steps (optimizer updates) to perform.")
    learning_rate: NonNegativeFloat = Field(2e-6, description="Optimizer's initial learning rate.")
    ppo_batch_size: PositiveInt = Field(
        4, description="Number of unique prompts to process in a single micro-batch during rollout generation."
    )
    num_rollout_samples: PositiveInt = Field(
        2, description="Number of distinct responses to generate per prompt for RL training."
    )
    grad_accum_steps: PositiveInt = Field(
        4, description="Number of micro-batches to accumulate gradients over before performing an optimizer step."
    )
    grpo_beta: NonNegativeFloat = Field(
        0.05, description="GRPO beta parameter, controlling the KL divergence penalty strength."
    )
    ...
    enable_dynamic_beta: bool = Field(False, description="Enable dynamic adjustment of GRPO beta based on training reward.")
    high_reward_threshold: float = Field(0.85, description="Reward threshold to trigger an increase in beta.")
    beta_increase_high_reward: float = Field(0.08, description="Amount to increase beta by when high reward is detected.")
    cooldown_duration: PositiveInt = Field(2, description="Number of updates to hold the adjusted beta before reverting.")
    early_train_steps: PositiveInt = Field(32, description="Number of initial training steps with reduced generation length/KV size for stability.")
    ...
    save_optimizer_state: bool = Field(False, description="Save optimizer state with checkpoints.")

    # Learning Rate Schedule
    lr_schedule_config: Dict[str, Any] = Field(
        default_factory=lambda: {
            "name": "cosine_decay",
            "arguments": [2e-6, 20000, 2e-7], # These will be dynamically overwritten by learning_rate, num_training_steps
            "warmup": 500,
            "warmup_init": 2e-7,
        },
    ...
    grad_checkpoint_layers: PositiveInt = Field(
        0, description="Number of layers to apply gradient checkpointing to (0 for all applicable layers)."
    )
    ...
    low_mul: NonNegativeFloat = Field(0.10, description="Gradient multiplier for layers in the low band.")
    mid_mul: NonNegativeFloat = Field(0.95, description="Gradient multiplier for layers in the mid band.")
    top_mul: NonNegativeFloat = Field(1.5, description="Gradient multiplier for layers in the top band.")
    head_mul: NonNegativeFloat = Field(1.2, description="Gradient multiplier for the LM head layer.")
    train_layer_start: int = Field(26, description="Starting layer index (inclusive) for which gradients are actively updated.")
    train_layer_end: int = Field(35, description="Ending layer index (inclusive) for which gradients are actively updated.")

    # Custom Invalid Sample Handling (Robustness Training)
    use_custom_batch_builder: bool = Field(
        False, description="Enable a custom batching strategy to include 'invalid' samples periodically."
    )
    ...
    invalid_sample_frequency: PositiveInt = Field(
        2, description="Frequency (every N updates) to incorporate 'invalid' samples in a dual-update step."
    )
    ...
    eval_every: PositiveInt = Field(250, description="Run full evaluation every N training updates.")
    benchmark_every: PositiveInt = Field(0, description="Run a quick benchmark every N updates (0 to disable).")

    # Monitoring (specific fields from original TrainingArgs)
    reward_smoothing_window: PositiveInt = Field(20, description="Window size for calculating a rolling average of rewards for display.")

    # Generation parameters now moved to GenerationConfig
    # ...
...
class ExperimentConfig(BaseModel):
    ...
    evaluation: List[EvaluatorConfig] = Field(
        default_factory=list, description="List of evaluator configurations for benchmarks."
    )
    ...
    kv_bits: Optional[PositiveInt] = Field(
        None, description="Quantization bits for KV cache (e.g., 4, 8). None for disabled."
    )
    ...
    run_server: bool = Field(
        False, description="If true, the script will start an async inference server instead of training."
    )
    use_paged_kv_cache: bool = Field(
        False, description="Enable the PagedKVCache for efficient memory management during rollouts/serving."
    )
    ...
    align_bridge_path: Optional[Path] = Field(
        None, description="Path to a pre-trained alignment bridge model's weights if cross-arch ref is enabled."
    )
    align_bridge_weight: NonNegativeFloat = Field(
        1.0, description="Weighting factor for the alignment bridge's reward/KL contribution."
    )
    align_pool: Literal["mean", "last"] = Field(
        "mean", description="Pooling method for alignment bridge embeddings ('mean' or 'last')."
    )
    align_after_tag: str = Field(
        "</think>", description="The tag after which text should be extracted for alignment computation."
    )

    # Benchmark specific settings
    benchmark_every: PositiveInt = Field(0, description="Run a quick benchmark evaluation every N updates (0 to disable).")
    benchmark_dataset: str = Field("gsm8k", description="Name of the Hugging Face dataset for quick benchmarking.")
    benchmark_dataset_config: Optional[str] = Field("main", description="Configuration for the benchmark dataset.")
    benchmark_split: str = Field("test", description="Dataset split to use for benchmarking (e.g., 'test', 'validation').")
    benchmark_prompt_key: str = Field("question", description="Key for prompt in benchmark dataset.")
    benchmark_answer_key: str = Field("answer", description="Key for answer in benchmark dataset.")
    benchmark_samples: PositiveInt = Field(10, description="Number of samples to use for the quick benchmark.")
    benchmark_max_new_tokens: PositiveInt = Field(196, description="Maximum new tokens to generate for benchmark solutions.")
    benchmark_temperature: NonNegativeFloat = Field(0.0, description="Sampling temperature for benchmark generation (0.0 for greedy).")
    benchmark_top_p: NonNegativeFloat = Field(1.0, description="Top-p sampling for benchmark generation.")
    benchmark_top_k: int = Field(0, description="Top-k sampling for benchmark generation (0 to disable).")
    benchmark_use_chat_template: bool = Field(True, description="Whether to apply chat template for benchmark prompts.")
    benchmark_stop_on_error: bool = Field(False, description="Stop benchmark if any single sample causes an error.")
    ...
    def load_from_yaml(cls, path: Path) -> "ExperimentConfig":
        ...
        try:
            ...
            if "trainer" in raw_config and "effective_batch_size" in raw_config["trainer"]:
