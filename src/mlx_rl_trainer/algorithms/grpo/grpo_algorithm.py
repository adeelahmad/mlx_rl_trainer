...
class GRPOAlgorithm(BaseAlgorithm):
    ...
    def calculate_loss_and_grads(
        ...
        def loss_fn(model):
            tokens, mask, advantages, ref_log_probs = (
                rollout_batch.get(k)
                for k in ["tokens", "response_mask", "advantages", "ref_log_probs"]
            )
            ...
            if (
                ...
                or advantages.size == 0
                or ref_log_probs is None
                or ref_log_probs.size == 0
            ):
            ...
            cur_lp = mx.take_along_axis(
                cur_lp, targets_resp[..., None], axis=-1
            ).squeeze(-1)
    ...
    def compute_advantages(
        ...
        if rewards_flat.shape[0] % samples_per_prompt != 0:
            logger.warning(
                "Rewards flat length not multiple of samples per prompt. Truncating for advantage calculation."
            )
        ...
        std = (
            mx.std(rewards_grouped, axis=1, ddof=0, keepdims=True) + 1e-8
