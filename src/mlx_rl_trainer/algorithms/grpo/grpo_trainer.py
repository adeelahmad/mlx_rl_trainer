...
class GRPOTrainer(BaseTrainer):
    ...
    def _setup(self) -> Tuple[int, int]:
        ...
        if (
            not actor_path.exists()
            or not actor_path.is_dir()
            or not (actor_path / "config.json").exists()
        ):
            ...
            logger.warning(
                f"Actor model path '{self.config.model.model_path}' not found or invalid. Using mock model at '{actor_path}'."
            )
        ...
        if (
            not ref_model_path.exists()
            or not ref_model_path.is_dir()
            or not (ref_model_path / "config.json").exists()
        ):
            ref_model_path = Path("./models/mock_model")
            logger.warning(
                f"Reference model path '{self.config.model.ref_model_path}' not found or invalid. Using mock model at '{ref_model_path}'."
            )
    ...
    def generate_rollouts(
        ...
        for step in range(self.config.data.max_gen_len):
            ...
            token_to_add = mx.where(
                ended_prev, self.tokenizer.pad_token_id, sampled_tok
            )
    ...
    def train_step(
        ...
        if not all(
            [self.optimizer_is_set, self.lr_scheduler_is_set, self.actor_model_is_set]
        ):
        ...
        metal_safe_apply_gradients(
            self.optimizer, grads, self.actor_model.trainable_parameters()
        )
        mx.eval(self.actor_model.parameters(), self.optimizer.state)

        grad_norm_val = mx.linalg.norm(
            mx.concatenate([mx.flatten(g) for g in mlx.utils.tree_leaves(grads)])
        )

        total_tokens = sum(s["input_ids"].size for s in batch_prompts_data) + int(
            mx.sum(rollout_batch["response_mask"]).item()
        )
        ...
        return TrainingMetrics(
            loss=loss.item(),
            reward_mean=avg_raw_reward,
            reward_std=float(mx.std(rollout_batch["raw_rewards"]).item()),
            kl_divergence=metrics.get("kl_divergence", 0.0),
            grad_norm=float(grad_norm_val.item()),
            learning_rate=lr,
            epoch=epoch,
            step=update_step,
