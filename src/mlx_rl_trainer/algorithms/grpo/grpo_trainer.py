...
try:
    ...
    logger.warning(
        "psutil not installed. Memory safety checks will be less accurate. "
...
class MemoryMonitor:
    ...
    def check_safety(self, ref_completion_length: int = 0) -> Tuple[bool, str]:
        ...
        if available < self.safety_threshold_mb:
            return False, (
                f"Low system memory: {available:.1f}MB available, "
...
class GRPOTrainer(BaseTrainer):
    ...
    def _calculate_grad_norm(self, grads) -> float:
        """
        Calculate the global L2 norm of gradients.

        Args:
            grads: Gradient tree (nested dict/list of arrays)
        ...
        try:
            ...
            for path, grad in flat_grads:
                if isinstance(grad, mx.array):
                    # Add squared norm of this gradient
                    total_norm_sq = total_norm_sq + mx.sum(grad * grad)

            # Take square root to get L2 norm
            grad_norm = float(mx.sqrt(total_norm_sq).item())

            # Cleanup
            del total_norm_sq, flat_grads
    ...
    def train_stepx(self, rollout_batch: Dict, update_step: int):
        """Execute one training step with aggressive memory management."""
        start_time = time.time()
        step_metrics = {}

        has_dual_masks = (
            "thinking_mask" in rollout_batch and "answer_mask" in rollout_batch
        )
        use_sft = (
            has_dual_masks
            and hasattr(self.config.trainer, "use_sft_on_answer")
            and self.config.trainer.use_sft_on_answer
            and "reference_tokens" in rollout_batch
        )
        ...
        if has_dual_masks:
            ...
            total_tokens = int(
                mx.sum(rollout_batch.get("response_mask", mx.array([0]))).item()
            )
        ...
        if (
            ...
            answer_start = getattr(
                self.config.trainer, "answer_layer_start", thinking_end + 1
            )
                    ...
                    step_metrics.update(
                        {
                            ...
                            "training/answer_weight_boost_ratio": answer_weight
                            / base_answer_weight,
            ...
            if use_sft:
                (
                    ...
                    rollout_batch,
                    rollout_batch["reference_tokens"],
                    self.config,
                    self.tokenizer.pad_token_id,
                )

                weighted_answer_grads = tree_map(
                    lambda g: g * answer_weight, answer_grads
                )

                if sft_grads:
                    weighted_sft_grads = tree_map(lambda g: g * sft_weight, sft_grads)
                    combined_answer_grads = safe_tree_add(
                        weighted_answer_grads, weighted_sft_grads
                    )
            ...
            # Mask to layer bands
            # Add import at top of train_step method
            ...
            if (
                ...
                # Unpack 6 values including metadata
                (
                    thinking_loss,
                    thinking_grads,
                    answer_loss,
                ...
                # Check metadata and log warnings
                if not dual_metadata["success"]:
                    logger.warning(
                        f"Dual gradient computation issues: {dual_metadata['structure_issues']}"
                    )

                if dual_metadata.get("match_rate", 1.0) < 0.8:
                    logger.warning(
                        f"Low gradient structure match rate: {dual_metadata['match_rate']:.1%}"
                    )
                ...
                logger.debug(
                    ...
                    f"(weight={thinking_weight}), "
                    f"Answer: layers {answer_start}-{answer_end} (weight={answer_weight})"
                )

                # Use safe gradient combination with metadata
                rl_grads, combine_metadata = _safe_gradient_combine(
                    thinking_grads,
                    answer_grads,
                    operation="add",
                ...
                # Check combination success
                if not combine_metadata["success"]:
                    logger.error(
                        f"Gradient combination failed: {combine_metadata['structure_issues']}"
                    )
                    # Fallback to thinking grads only
                    logger.warning("Falling back to thinking gradients only")
                    rl_grads = thinking_grads
                elif combine_metadata.get("match_rate", 1.0) < 0.8:
                    logger.warning(
                        f"Gradient combination partial match: {combine_metadata['match_rate']:.1%}"
                    )
                ...
                # Standard single gradient computation
                loss, rl_grads, metrics = self.grpo_algorithm.calculate_loss_and_grads(
                    rollout_batch, self.config, self.tokenizer.pad_token_id
                )

            # SFT combination (if enabled)
            if (
                hasattr(self.config.trainer, "sft_weight")
                and self.config.trainer.sft_weight > 0
            ):
                if "reference_tokens" in rollout_batch:
                    (
                        sft_loss,
                        sft_grads,
                        sft_metrics,
                    ...
                    # Update metrics with SFT
                    metrics.update(sft_metrics)

                    # Safe combination with SFT gradients
                    sft_weight = self.config.trainer.sft_weight

                    final_grads, sft_combine_metadata = _safe_gradient_combine(
                        rl_grads,
                        sft_grads,
                        operation="add",
                        weight1=1.0,
                    ...
                    # Check SFT combination
                    if not sft_combine_metadata["success"]:
                        logger.error(
                            f"SFT gradient combination failed: {sft_combine_metadata['structure_issues']}"
                        )
                        logger.warning("Using RL gradients only")
                        grads = rl_grads
                    else:
                        grads = final_grads
                        if sft_combine_metadata.get("match_rate", 1.0) < 0.8:
                            logger.warning(
                                f"SFT combination partial match: {sft_combine_metadata['match_rate']:.1%}"
                            )
            ...
            # Final validation before optimizer update
            if not _validate_gradient_dict(grads, "final gradients"):
                logger.error("Final gradients are invalid! Skipping optimizer update.")
                # Return safe metrics without updating
                return (
                    TrainMetrics(
            ...
            # Apply gradients
            self.optimizer.update(self.actor_model, grads)
    ...
    def train_step(self, rollout_batch: Dict, update_step: int):
        ...
        # Initialize final_grads to None (will be set in all branches)
        final_grads = None
        total_loss = 0.0
        ...
        if (
            ...
            if "reference_tokens" in rollout_batch:
                ...
                step_metrics["loss/answer_sft_loss"] = float(sft_loss.item())
            else:
                logger.warning("SFT enabled but no reference_tokens in batch")
                final_grads = rl_grads
        else:
            # No SFT, use RL gradients directly
            final_grads = rl_grads

        # Final validation before optimizer update
        if not _validate_gradient_dict(final_grads, "final gradients"):
            logger.error("Final gradients are invalid! Skipping optimizer update.")
            # Return safe metrics without updating
            return (
                TrainingMetrics(
        ...
        grad_accum = self.config.trainer.grad_accum_steps
        if grad_accum > 1:
            final_grads = tree_map(lambda g: g / grad_accum, final_grads)

        # Apply gradients
        self.optimizer.update(self.actor_model, final_grads)
        mx.eval(self.actor_model.parameters(), self.optimizer.state)
        ...
        if has_dual_masks:
            self.chart_data["token_history"].append(
                {
                    "step": update_step,
                    "thinking": step_metrics.get("training/thinking_token_count", 0),
                    "answer": step_metrics.get("training/answer_token_count", 0),
                }
            )
            if len(self.chart_data["token_history"]) > self._max_history:
                del self.chart_data["token_history"][0]
    ...
    def generate_rollouts(self, batch_data: Dict, update_step: int):
        ...
        try:
            ...
            is_invalid = any(
                data.get("is_invalid_sample", False) for data in prompts_data
            )
    ...
    def log_comprehensive_metrics(
        self, step: int, step_metrics: Dict, generation_metrics: Optional[Dict] = None
    ):
