...
def _execute_code_in_isolated_process_wrapper(
    ...
    It deserializes config and test_cases, then calls the actual execution logic.
    """
    ...
    try:
        ...
        if (
            not test_cases
        ):  # For benchmarks like MBPP that have tests in the code itself
            if _validate_syntax_isolated(code):
                ...
                if process_result.returncode == 0:
                    result_queue.put(1.0)  # Assume success if it runs without error
                else:
                    logger.warning(
    ...
    def compute(self, context: RewardContext) -> float:
        ...
        try:
            process = mp_context.Process(
                target=_execute_code_in_isolated_process_wrapper,
                args=(
                    code,
            ...
            for original_idx, future, result_queue in futures_with_indices:
                try:
                    ...
                    rewards_for_batch[original_idx] = {
                        self.name: float(max(0.0, min(1.0, final_score))),
                        "total": final_score,
                    }
                except Exception as e:
                    logger.error(
