...
from mlx_rl_trainer.core.model_manager import MockTokenizer # For tokenizing during preprocessing
from mlx_rl_trainer.data.dataset_manager import _normalize_record, _contains_keywords, _looks_garbage # Reuse helpers
from mlx_rl_trainer.utils.text_utils import apply_chat_template_wrapper # For consistent chat formatting

logger = logging.getLogger(__name__)


async def _async_load_and_process_jsonl(path: Path, prompt_key: str, answer_key: str, filter_keywords: List[str], system_prompt: str, tokenizer: MockTokenizer) -> List[Dict[str, Any]]:
            ...
            if line.strip():
                try:
                    ...
                    if normalized_rec:
                        ...
                        s = normalized_rec["system"] if normalized_rec["system"] else system_prompt
                        tc = normalized_rec["test_cases"] # test_cases are already list of dicts
                        meta = normalized_rec["meta"]

                        # Apply filters
                        if filter_keywords and (
                            _contains_keywords(p, filter_keywords) or
                            _contains_keywords(c, filter_keywords)
                        ):
                            continue
                        ...
                        encoded_prompt_len = len(tokenizer.encode(formatted_prompt, add_special_tokens=True))
                    ...
                    logger.warning(f"Skipping malformed JSONL line in {path}: {line[:80]}...")
                except Exception as e:
                    logger.warning(f"Error processing line from {path}: {e}. Skipping.", exc_info=True)
    return processed_data


async def _async_load_and_process_hf(dataset_name: str, dataset_config: Optional[str], split: str, prompt_key: str, answer_key: str, filter_keywords: List[str], system_prompt: str, tokenizer: MockTokenizer) -> List[Dict[str, Any]]:
    """Asynchronously loads, normalizes, filters, and tokenizes a HuggingFace dataset."""
    try:
        ds = await asyncio.to_thread(hf_load_dataset, dataset_name, dataset_config, split=split)
        ...
        raise DataLoadError(f"Async HF dataset load and process failed for {dataset_name}: {e}") from e
...
async def preprocess_and_save_dataset(
    ...
    tokenizer = MockTokenizer() # Use mock tokenizer for preprocessing token length estimates
    ...
    if train_path.suffix.lower() in ('.jsonl', '.ndjson'):
        tasks.append(_async_load_and_process_jsonl(train_path, config.data.dataset_prompt_key, config.data.dataset_answer_key, config.data.dataset_filter_keywords, config.system_prompt, tokenizer))
    else: # Assume HF dataset
        tasks.append(_async_load_and_process_hf(train_path.as_posix(), config.data.get("dataset_config"), config.data.get("dataset_train_split", "train"), config.data.dataset_prompt_key, config.data.dataset_answer_key, config.data.dataset_filter_keywords, config.system_prompt, tokenizer))

    # Val data
    if config.data.val_path:
        val_path = config.data.val_path
        if val_path.suffix.lower() in ('.jsonl', '.ndjson'):
            tasks.append(_async_load_and_process_jsonl(val_path, config.data.dataset_prompt_key, config.data.dataset_answer_key, config.data.dataset_filter_keywords, config.system_prompt, tokenizer))
        else: # Assume HF dataset
            tasks.append(_async_load_and_process_hf(val_path.as_posix(), config.data.get("dataset_config"), config.data.get("dataset_val_split", "test"), config.data.dataset_prompt_key, config.data.dataset_answer_key, config.data.dataset_filter_keywords, config.system_prompt, tokenizer))
    ...
    common_features = Features({
        ...
        "meta": {"is_mcq": Value("bool"), "options": [Value("string")], "correct_letters": Value("string"), "multi_select": Value("bool"), "correct_indices": [Value("int32")]}, # Nested meta
    ...
    val_dataset = Dataset.from_list(val_processed_data, features=common_features) if val_processed_data else None

    # Save processed datasets
    output_train_path.parent.mkdir(parents=True, exist_ok=True)
    train_dataset.to_json(output_train_path, indent=2)
    logger.info(f"Processed training dataset saved to: {output_train_path} ({len(train_dataset)} samples).")

    if val_dataset:
        output_val_path.parent.mkdir(parents=True, exist_ok=True)
        val_dataset.to_json(output_val_path, indent=2)
        logger.info(f"Processed validation dataset saved to: {output_val_path} ({len(val_dataset)} samples).")

    logger.info("Dataset preprocessing completed.")


def main():
    parser = argparse.ArgumentParser(description="MLX RL Trainer - Data Preprocessing Script")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
    ...
    # Run preprocessing
    try:
        asyncio.run(preprocess_and_save_dataset(
            exp_config,
