...
from mlx_rl_trainer.core.trainer import TrainingRuntimeError, CustomBaseException, ModelLoadError, DataLoadError # Exceptions
...
from mlx_rl_trainer.utils.mlx_utils import metal_safe_apply_gradients # For Metal safety
from mlx_rl_trainer.utils.text_utils import _ensure_schedule_dict # For LR schedule init
from mlx_rl_trainer.generation.caching import PagedKVCache # For PagedKV (from correct path)
...
def main():
    ...
    except (FileNotFoundError, ValueError, Exception) as e: # Catch Pydantic ValidationError also
    ...
    if not Path(config.checkpointing.save_dir).is_absolute():
        config.checkpointing.save_dir = str(config.trainer.output_dir / config.checkpointing.save_dir)
    ...
    logger.info(f"Starting run with ID: [bold magenta]{run_id}[/bold magenta], seed: {config.trainer.seed}")
    ...
    signal.signal(signal.SIGINT, lambda s, f: globals().__setitem__('shutdown_requested', True)) # Simple lambda for global flag
    signal.signal(signal.SIGTERM, lambda s, f: globals().__setitem__('shutdown_requested', True))
    ...
    for reward_cfg in config.rewards:
        ...
        reward_cfg.config['think_open_tag'] = config.rewards[0].think_start_tag if config.rewards else '<think>'
        reward_cfg.config['think_close_tag'] = config.rewards[0].think_end_tag if config.rewards else '</think>'
        reward_cfg.config['answer_open_tag'] = config.rewards[0].answer_start_tag if config.rewards else '<answer>'
        reward_cfg.config['answer_close_tag'] = config.rewards[0].answer_end_tag if config.rewards else '</answer>'
    ...
    data_manager = DatasetManager(config.data, tokenizer=None) # Tokenizer is set in trainer setup
    checkpoint_manager = CheckpointManager(Path(config.checkpointing.save_dir),
    ...
    if config.use_paged_kv_cache:
        try:
            ...
            if model_config_path.exists():
                ...
                logger.warning(f"Could not find model config at {model_config_path}. Using hardcoded defaults for PagedKVCache init.")

            paged_kv_cache = PagedKVCache(
                num_layers=mock_model_config.get("num_hidden_layers", 4),
                num_kv_heads=mock_model_config.get("num_key_value_heads", 2),
                head_dim=mock_model_config.get("hidden_size", 128) // mock_model_config.get("num_attention_heads", 4),
    ...
    if config.trainer.algorithm == "grpo":
        trainer = GRPOTrainer(config, model_manager, data_manager, checkpoint_manager, reward_composer, paged_kv_cache)
    else:
...
if __name__ == "__main__":
    mx.set_default_device(mx.gpu if mx.gpu_available() else mx.cpu) # Set default device
    main()
